...
replace_text() {
    ...
    if ! sed $options "s/${source_text//\//\\/}/${replace_text//\//\\/}/g" "$file" > $temp_sed; then
        echo -e "${COLOR_RED}错误：无法将文件 '$file' 中符合 '$source_text' 的文本替换为 '$replace_text'。${COLOR_RESET}" >&2
        return 1
    fi

    if cmp -s "$file" "$temp_sed"; then
        echo -e "${COLOR_RED}错误：未在文件 '$file' 中查找到符合 '$source_text' 的文本。${COLOR_RESET}" >&2
    else
        mv "$temp_sed" "$file"
        echo "成功将文件 '$file' 中符合 '$source_text' 的文本替换为 '$replace_text'。"
    fi
}
...
replace_text "gale/src/lib/menu/GameSelection.svelte" "Your game missing? If the game is new on Thunderstore there's a chance we have yet to add" "没找到您想玩的游戏？如果这个游戏刚刚发布，则 Thunderstore 可能还没有添加它。"
replace_text "gale/src/lib/menu/GameSelection.svelte" "it\. If you can find it on" "如果您能在"
...
replace_text "gale/src/lib/config/ConfigEntryField.svelte" "'Integer'" "'整数'"
replace_text "gale/src/lib/config/ConfigEntryField.svelte" "'Decimal'" "'浮点数'"
replace_text "gale/src/lib/config/ConfigEntryField.svelte" "'String'" "'字符串'"
replace_text "gale/src/lib/config/ConfigEntryField.svelte" "'Boolean'" "'布尔值'"
replace_text "gale/src/lib/config/ConfigEntryField.svelte" "'Enum'" "'枚举'"
replace_text "gale/src/lib/config/ConfigEntryField.svelte" "'Flags'" "'标志'"
...
replace_text "gale/src/lib/config/ExpandedEntryPopup.svelte" "title=\"Edit" "title=\"编辑"
replace_text "gale/src/lib/config/ExpandedEntryPopup.svelte" "Edit as text" "以文本形式编辑"
replace_text "gale/src/lib/config/ExpandedEntryPopup.svelte" "Edit as list" "以列表形式编辑"
replace_text "gale/src/lib/config/ExpandedEntryPopup.svelte" "Enter new value\.\.\." "请输入新值…"
replace_text "gale/src/lib/config/ResetConfigButton.svelte" "Are you sure you want to reset \(.\+\)?" "您真的确定要重置 \1 吗？"
...
replace_text "gale/src/lib/import/ExportCodePopup.svelte" "Export complete! The code has been copied to your clipboard:" "导出完成！代码已经复制到您的剪切板中："
replace_text "gale/src/lib/import/ImportProfilePopup.svelte" "Are you sure you want to override \(.\+\)?" "真的确定要覆盖 \1？"
replace_text "gale/src/lib/import/ImportProfilePopup.svelte" "Imported profile \(.\+\)\." "已导入整合包 \1。"
...
replace_text "gale/src/lib/import/ImportProfilePopup.svelte" "This is unsafe and can let an attacker install malware on your system\." "这是一种不安全的操作，可能会让攻击者在您的系统上安装恶意软件。"
...
replace_text "gale/src/lib/import/ImportProfilePopup.svelte" "Owned by " "所有人："
...
replace_text "gale/src/lib/menu/AboutPopup.svelte" "GitHub</Link>\n\(\t\+\)</div>" 'GitHub</Link>\n\1</div>\n\1<div class="mt-1 flex items-center gap-2">\n\1\t<Icon icon="mdi:ideogram-cjk-variant" class="text-xl text-white" />\n\1\t<Link href="https://github.com/SummonHIM/gale-chs">GitHub</Link>\n\1</div>' 1
...
replace_text "gale/src/lib/menu/Contextbar.svelte" "Are you sure you want to delete \(.\+\)?" "您真的确定要删除 \1？"
replace_text "gale/src/lib/menu/Contextbar.svelte" "Deleted profile \(.\+\)\." "已删除整合包 \1。"
...
replace_text "gale/src/lib/menu/Menubar.svelte" "Imported local mod into profile." "本地模组已导入到整合包中。"
...
replace_text "gale/src/lib/menu/Menubar.svelte" "Are you sure you want to uninstall all disabled mods?" "您真的确定要卸载所有已禁用的模组吗？"
...
replace_text "gale/src/lib/menu/Menubar.svelte" "Are you sure you want to delete all cached mods? This could potentially double the disk space used by installed mods\. Only proceed if you know what you're doing!" "您真的确定要删除所有已缓存的模组吗？这可能会造成已安装的模组所使用的磁盘空间增加一倍。请确认好是否真的要这么做！"
replace_text "gale/src/lib/menu/Menubar.svelte" "Deleted\${\(.\+\) ? ' unused' : ''} mod cache (cleared \(.\+\))." "已删除\${\1 ? '未使用的' : ''}模组缓存（已清理 \2）。"
replace_text "gale/src/lib/menu/Menubar.svelte" "Copied mod list to clipboard." "已复制模组列表至剪切板。"
replace_text "gale/src/lib/menu/Menubar.svelte" "Copied debug info to clipboard." "已复制调试信息至剪切板。"
...
replace_text "gale/src/lib/menu/Syncer.svelte" "Sync off" "关闭同步"
replace_text "gale/src/lib/menu/Syncer.svelte" "Up to date" "已是最新"
replace_text "gale/src/lib/menu/Syncer.svelte" "Outdated" "需要更新"
replace_text "gale/src/lib/menu/Syncer.svelte" "Created synced profile\." "已创建同步整合包。"
replace_text "gale/src/lib/menu/Syncer.svelte" "Pushed update to synced profile\." "已将本地整合包内容同步到云端。"
replace_text "gale/src/lib/menu/Syncer.svelte" "Pulled changes from synced profile\." "已将云端整合包内容同步到本地。"
replace_text "gale/src/lib/menu/Syncer.svelte" "Refresh synced profile status\." "已刷新同步整合包的状态。"
replace_text "gale/src/lib/menu/Syncer.svelte" "Do you also want to delete the profile from the database?" "请问您还要从云端数据库中删除该整合包吗？"
replace_text "gale/src/lib/menu/Syncer.svelte" "Disconnected synced profile\." "已断开同步整合包。"
replace_text "gale/src/lib/menu/Syncer.svelte" "Profile sync" "整合包同步"
replace_text "gale/src/lib/menu/Syncer.svelte" "Owned by " "所有人："
replace_text "gale/src/lib/menu/Syncer.svelte" "Copy to clipboard" "复制到剪切板"
replace_text "gale/src/lib/menu/Syncer.svelte" "Copied profile code to clipboard\." "已复制整合包代码至剪切板。"
replace_text "gale/src/lib/menu/Syncer.svelte" "Pull update" "拉取更新"
replace_text "gale/src/lib/menu/Syncer.svelte" "Push update" "推送更新"
replace_text "gale/src/lib/menu/Syncer.svelte" "\tRefresh" "\t刷新"
replace_text "gale/src/lib/menu/Syncer.svelte" "\tDisconnect" "\t断开连接"
replace_text "gale/src/lib/menu/Syncer.svelte" "\tConnect" "\t连接"
replace_text "gale/src/lib/menu/Syncer.svelte" "You must be logged in to create a synced profile\." "您需要先登录才能创建同步整合包"
replace_text "gale/src/lib/menu/Syncer.svelte" "Login with Discord" "通过 Discord 登录"
replace_text "gale/src/lib/menu/Syncer.svelte" "Log out" "登出"
replace_text "gale/src/lib/menu/Syncer.svelte" "What is this?" "这是啥？"
...
replace_text "gale/src/lib/menu/WelcomePopup.svelte" "You can automatically transfer profiles from another mod manager to Gale\." "您可以选择从其他模组管理器自动导入整合包至 Gale。"
replace_text "gale/src/lib/menu/WelcomePopup.svelte" "You can always import profiles later by going to <b>Import \&gt; \.\.\.from r2modman</b>\." "您也可以等会在 <b>导入 \&gt; 从其他平台导入模组</b> 里自动导入整合包。"
...
replace_text "gale/src/lib/menu/WelcomePopup.svelte" "You can always edit these later by going to" "您可以随时在此处编辑以下内容："
...
replace_text "gale/src/lib/menu/WelcomePopup.svelte" "That's it, you're all set up to start modding!" "没别的了，您可以开始给您的游戏打模组了。"
...
replace_text "gale/src/lib/modlist/InstallModPopup.svelte" "title=\"Install" "title=\"安装"
replace_text "gale/src/lib/modlist/InstallModPopup.svelte" ">Install</BigButton>" ">安装</BigButton>"
replace_text "gale/src/lib/modlist/InstallModPopup.svelte" "Choose a profile to install the mod to:" "请选择要添加模组的整合包："
replace_text "gale/src/lib/modlist/InstallProgressPopup.svelte" "Installing mods" "正在安装模组"
replace_text "gale/src/lib/modlist/InstallProgressPopup.svelte" "Are you sure you want to abort the installation?" "您真的确定要终止安装吗？"
...
replace_text "gale/src/lib/modlist/ModDetails.svelte" "Last updated \(.\+\) ago" "上次更新在\1前"
...
replace_text "gale/src/lib/modlist/ProfileLockedBanner.svelte" 'Profile is locked' '整合包已锁定'
...
replace_text "gale/src/lib/prefs/ApiKeyPopup.svelte" ". Login to" "1\. 登录到"
replace_text "gale/src/lib/prefs/ApiKeyPopup.svelte" ". Go to" "2\. 前去"
...
replace_text "gale/src/lib/util.ts" "Math.round(seconds)} seconds" "Math.round(seconds)}秒"
replace_text "gale/src/lib/util.ts" " minute\${\(.\+\) > 1 ? 's' : ''}" "分钟"
replace_text "gale/src/lib/util.ts" " hour\${\(.\+\) > 1 ? 's' : ''}" "小时"
replace_text "gale/src/routes/browse/+page.svelte" "Profile locked" "整合包已锁定"
...
replace_text "gale/src/routes/browse/+page.svelte" "Try to adjust your search query/filters" "尝试调整您的过滤和搜索"
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Name\"" "label=\"名称\""
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Author\"" "label=\"作者\""
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Description\"" "label=\"介绍\""
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Categories\"" "label=\"分类\""
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Version\"" "label=\"版本号\""
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Website\"" "label=\"网站\""
...
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Readme\"" "label=\"详细介绍\""
...
replace_text "gale/src/routes/modpack/+page.svelte" ">Preview</summary>" ">预览</summary>"
replace_text "gale/src/routes/modpack/+page.svelte" "label=\"Changelog\"" "label=\"变更日志\""
...
replace_text "gale/src/routes/prefs/+page.svelte" "relatively up-to-date, but can be disabled to save bandwidth\." "该选项将确保模组列表始终为最新。但您可将其禁用来节省带宽。"
...
replace_text "gale/src/routes/prefs/+page.svelte" "Pull before launch" "启动时拉取"
replace_text "gale/src/routes/prefs/+page.svelte" "Whether to pull updates from synced profiles before launching\." "是否允许在启动前从整合包同步服务器拉取更新。"
...
replace_text "gale/src/routes/+page.svelte" "Try to adjust your search query/filters" "尝试调整您的过滤和搜索"
